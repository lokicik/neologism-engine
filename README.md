...
- **Description-driven naming** — simplified RAKE keyword extraction turns a product description into blend roots, with distinct offline semantic families for developer tools and common domains such as legal work, recruiting, meals, support, events, weather, habits, sales, and pet care
...
123 unit tests covering Markov determinism, phonotactic filters, blend logic, score ranges, phoneme affinity, sonority sequencing, word-likeness, keyword extraction, semantic ranking, exclusion behavior, developer-domain coverage, first-page shape balance, and 100-name brief sessions.
...
- **Project-scoped local taste selection** — star or pass on 3+ names; future batches for that project draw a 3× offline candidate pool and show the page closest to liked structures and naming modes or furthest from repeatedly rejected ones. Feedback from unrelated briefs never affects the current project and stays in `localStorage`.
