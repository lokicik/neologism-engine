...
- **Project-scoped local taste learning** — star or pass on 3+ names; future batches for that project are automatically re-ranked toward liked structures and naming modes or away from repeatedly rejected ones. Feedback from unrelated briefs never affects the current project and stays in `localStorage`.
