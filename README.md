...
108 unit tests covering Markov determinism, phonotactic filters, blend logic, score ranges, phoneme affinity, sonority sequencing, word-likeness, keyword extraction, semantic ranking, exclusion behavior, and 100-name brief sessions.
...
> Compound quality check: `cargo run -p neologism-core --example compound_compare --release` audits relevance, structural scores, seed diversity, and 100-name capacity across six briefs.
...
- **Brief-aware Compound mode** — readable two-word names use project-specific adjective palettes and semantic noun roots (`QuietInk`, `FairTally`, `SwiftSignal`) instead of arbitrary corpus pairings; audited multi-concept sessions reach 100 fresh names
