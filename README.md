...
> Cold Auto quality check: from `web/`, `node e2e/cold-quality-audit.mjs` audits 90 fixed pages, including the bounded weak-slot repair, structural floor, and within-page similarity.
...
- **Brief-aware Auto** — a project description gets semantic Brandable names plus at most one Respell that is actually derived from the prompt; unrelated modes no longer receive forced slots. On a cold page, strong names keep their order and a bounded offline fallback replaces only missing or sub-75 slots. An empty brief keeps the broader four-mode sampler.
