// One-shot 100k generation sweep: pre-33 vs current full defaults.
...
fn main() {
    println!("100k big-tech sweep — {BATCHES} batches × {COUNT} (variety 0.3, window {WINDOW})\n");
    let pre33 = BigTechTuning { fuzzy_exclude: false, stem_exclude: false, max_share: 1.0, ..BigTechTuning::from_variety(0.3) };
    run(&pre33, "pre-33");
    run(&BigTechTuning::from_variety(0.3), "Phase 34 full defaults");
}
