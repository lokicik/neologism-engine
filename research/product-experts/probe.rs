//! Frozen Phase 292 brief-conditioned product-of-experts research probe.
...
const BAD_SUBSTRINGS: &[&str] = &[
    ...
    "vomit", "poop", "defect", "fraud", "scam", "lousy", "kill", "mong",
];

const METAPHOR_TAILS: &[&str] = &[
    "flow", "forge", "spark", "seed", "craft", "nest", "lab", "wave", "link",
    "pulse", "beam", "grid", "vault", "relay", "trace", "scope", "prism", "lumen",
    "nova", "peak", "trail", "path", "signal", "hive", "smith", "harbor", "grove",
    "spring", "frame", "glow", "flux", "loom", "muse", "atlas",
];

#[derive(Clone)]
struct CharModel {
    counts: HashMap<[u8; 2], [u32; 27]>,
    totals: HashMap<[u8; 2], u32>,
}

impl CharModel {
    fn train<'a>(names: impl IntoIterator<Item = &'a str>) -> Self {
        let mut counts: HashMap<[u8; 2], [u32; 27]> = HashMap::new();
        let mut totals: HashMap<[u8; 2], u32> = HashMap::new();
        for name in names {
            ...
            for window in sequence.windows(3) {
                ...
                counts.entry(context).or_insert([0; 27])[index] += 1;
    ...
    fn sample(&self, semantic: &Self, rng: &mut ChaCha8Rng) -> Option<String> {
        let mut context = [b'^', b'^'];
        ...
        loop {
            ...
            let mut weighted: Vec<(u8, f64)> = Vec::with_capacity(SYMBOLS.len());
            let mut total = 0.0;
            for &symbol in SYMBOLS {
                if symbol == b'$' && result.len() < 4 {
                    continue;
                }
                let log_weight = FORM_WEIGHT * self.probability(context, symbol).ln()
                    + SEMANTIC_WEIGHT * semantic.probability(context, symbol).ln();
                let weight = (log_weight / TEMPERATURE).exp();
                weighted.push((symbol, weight));
                total += weight;
            }
            let mut pick = rng.gen::<f64>() * total;
            let mut chosen = weighted.last()?.0;
...
impl CollisionIndex {
    fn new(values: impl IntoIterator<Item = String>) -> Self {
        let exact: HashSet<String> = values
            ...
            .filter(|value| !value.is_empty() && value.bytes().all(|byte| byte.is_ascii_lowercase()))
            .collect();
...
fn build_specs(briefs: Vec<String>) -> Vec<BriefSpec> {
    ...
    ordered
        ...
        .map(|text| {
            ...
            let groups: Vec<Vec<String>> = brand_root_groups(&keywords, 16)
                .into_iter()
                .map(|group| {
                    let mut roots: Vec<String> = group
                        .into_iter()
                        .map(|root| root.to_ascii_lowercase())
                        .filter(|root| {
                            !root.is_empty() && root.bytes().all(|byte| byte.is_ascii_lowercase())
                        })
                        .collect();
                    roots.sort();
                    roots.dedup();
            ...
            let models = groups
                .iter()
                .map(|group| CharModel::train(group.iter().map(String::as_str)))
                .collect();
...
fn select(pool: &[Candidate], group_count: usize) -> Vec<Candidate> {
    if pool.is_empty() {
        return Vec::new();
    }
    let min_global = pool
    ...
    while selected.len() < PAGE_SIZE {
        ...
        for &index in &remaining {
            ...
            let max_similarity = selected
                .iter()
                .map(|selected_index| {
                    similarity(&pool[index].name, &pool[*selected_index].name)
                })
                .fold(0.0, f64::max);
    ...
    selected.into_iter().map(|index| pool[index].clone()).collect()
}
...
fn generate_page(
    ...
    let mut rng = ChaCha8Rng::seed_from_u64(seed ^ fnv1a64(&spec.text));
    let mut attempts = 0usize;
    let mut rejections = Rejections::new();
    let mut seen = HashSet::new();
    let mut pool = Vec::new();
    while attempts < MAX_ATTEMPTS && pool.len() < POOL_SIZE && !spec.models.is_empty() {
        let source_group = attempts % spec.models.len();
        attempts += 1;
        let Some(lower) = global.sample(&spec.models[source_group], &mut rng) else {
        ...
        pool.push(Candidate {
            ...
            origin: "product_experts",
        });
    }
    pool.sort_by(|left, right| left.name.cmp(&right.name));
    let selected = select(&pool, spec.models.len());
...
fn summarize(pages: &[Page], form_floor: f64, replay: bool) -> (serde_json::Value, BTreeMap<String, bool>) {
    let selected: Vec<&Candidate> = pages.iter().flat_map(|page| page.selected.iter()).collect();
    let total = selected.len();
    let quality_sum: u64 = selected.iter().map(|candidate| candidate.composite as u64).sum();
    ...
    gates.insert("all_pools_160".into(), pages.iter().all(|page| page.pool.len() == POOL_SIZE));
    gates.insert("all_pages_10".into(), pages.iter().all(|page| page.selected.len() == PAGE_SIZE));
    gates.insert("all_form_floor".into(), all_form_floor);
    gates.insert("average_quality_at_least_84".into(), average_quality >= 84.0);
    gates.insert("condition_win_rate_at_least_70pct".into(), condition_win_rate >= 0.70);
    gates.insert("duplicate_page_sets_zero".into(), duplicate_page_sets == 0);
    gates.insert("mean_diversity_at_least_0_72".into(), mean_diversity >= 0.72);
    gates.insert("mean_overlap_at_most_1".into(), mean_overlap <= 1.0);
    gates.insert("minimum_brief_unique_at_least_27".into(), minimum_brief_unique >= 27);
    gates.insert("minimum_diversity_at_least_0_60".into(), minimum_diversity >= 0.60);
    ...
    gates.insert("template_tail_rate_at_most_20pct".into(), template_tail_rate <= 0.20);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let specs = build_specs(brief_values);
    ...
    let report = Report {
        schema: "neologism-product-experts-report-v1",
