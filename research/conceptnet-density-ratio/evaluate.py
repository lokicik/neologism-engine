#!/usr/bin/env python3
"""Run the frozen Phase 301 density-ratio selector preflight."""
...
def background_model(path: Path) -> CharModel:
...
def enriched_pool(page: dict[str, object], background: CharModel) -> list[dict[str, object]]:
    result = []
    for raw in page["pool"]:
        candidate = dict(raw)
        lower = str(candidate["name"]).lower()
        background_logp = background.log_likelihood(lower)
        lift = float(candidate["semantic_logp"]) - background_logp
        candidate["background_anchor_logp"] = background_logp
        candidate["density_lift"] = lift
        if lift > 0.0:
...
def select(pool: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    min_lift = min(float(candidate["density_lift"]) for candidate in pool)
    max_lift = max(float(candidate["density_lift"]) for candidate in pool)
    relevance = [
        0.60 * float(candidate["composite"]) / 100.0
        + 0.20
        * normalize(float(candidate["global_logp"]), min_global, max_global)
        + 0.20 * normalize(float(candidate["density_lift"]), min_lift, max_lift)
...
def main() -> int:
    ...
    background = background_model(args.anchors)
    pages = []
    for source_page in source["pages"]:
        eligible = enriched_pool(source_page, background)
        selected = select(eligible)
        page = {
            "brief": source_page["brief"],
            "eligible_density_pool": len(eligible),
        ...
        if len(
            {
                ...
                for candidate in enriched_pool(source_page, background)
            }
        )
        >= 2
        else True
    ...
    gates = {
        "all_density_lifts_positive": all(float(candidate["density_lift"]) > 0.0 for candidate in selected),
    ...
    summary = {
        ...
        "mean_density_pool": sum(int(page["eligible_density_pool"]) for page in pages) / len(pages),
        "mean_diversity": mean_diversity,
        "mean_overlap": mean_overlap,
        "minimum_brief_unique": minimum_unique,
        "minimum_density_pool": min(int(page["eligible_density_pool"]) for page in pages),
    ...
    report = {
        ...
        "schema": "neologism-conceptnet-density-ratio-report-v1",
