...
import { root, dir } from './harness.mjs'
...
for (const script of scripts) {
  ...
  writeFileSync(resolve(dir, 'artifacts', `${script}.log`), result.output)
  results.push({ script, exitCode: result.code, durationMs: Date.now() - started })
  console.log(`${result.code === 0 ? 'PASS' : 'FAIL'} ${script} (${Math.round((Date.now() - started) / 1000)} s)`)
}
writeFileSync(resolve(dir, 'artifacts/verification.json'), JSON.stringify(results, null, 2) + '\n')
