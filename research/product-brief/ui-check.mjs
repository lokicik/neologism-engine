...
const out = resolve(root, 'research/product-brief/artifacts-v3')
await withBrowser(async (page) => {
  ...
  assert.equal(exported.run.config.variant, 'brief_pool')
  ...
  assert.equal(await page.evaluate(() => JSON.stringify(localStorage)), storage)
  // Exercise the newly supported alias and retained supporting words in the
  // actual app, independently of the generator-level regression harness.
  await page.getByRole('button', { name: /Brief intent/ }).click()
  await ready()
  await page.getByRole('checkbox', { name: /Use product benefits/ }).check()
  await page.getByRole('textbox', { name: 'Project brief' }).fill('a utility that recovers damaged archive entries from replicas')
  await page.getByRole('button', { name: 'Generate', exact: true }).click()
  await page.waitForFunction(() => document.querySelector('.candidate-lab')?.getAttribute('aria-busy') === 'false'
    && document.querySelector('.candidate-lab')?.textContent?.includes('Product: archive'), null, { timeout: 60000 })
  assert.match(await page.locator('.candidate-lab').innerText(), /Product: archive.*property: entries/)
  assert.match(await page.locator('.candidate-lab').innerText(), /Supporting words retained outside naming roots: damaged, replicas/)
  assert.match(await page.locator('.candidate-lab').innerText(), /Generation terms: restore, archive, entry/)
  assert(await page.locator('.finalist').count() > 0)
  await page.locator('.finalist-list').screenshot({ path: resolve(out, 'recovery-finalists.png'), animations: 'disabled' })
